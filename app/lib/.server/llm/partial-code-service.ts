...
export function parseCodeUpdateOperations(aiResponse: string): CodeUpdateOperation[] {
  ...
  while ((match = updateFunctionRegex.exec(aiResponse)) !== null) {
    const [_, filePath, functionName, newImplementation] = match;
  ...
  const updateMethodRegex = /<updateMethod\s+file="([^"]+)"\s+class="([^"]+)"\s+name="([^"]+)">([\s\S]*?)<\/updateMethod>/g;
  
  while ((match = updateMethodRegex.exec(aiResponse)) !== null) {
    const [_, filePath, className, methodName, newImplementation] = match;
  ...
  while ((match = updateClassRegex.exec(aiResponse)) !== null) {
    const [_, filePath, className, newImplementation] = match;
  ...
  while ((match = replaceFileRegex.exec(aiResponse)) !== null) {
    const [_, filePath, newImplementation] = match;
 ...
 */
export function applyCodeUpdateOperations(
  operations: CodeUpdateOperation[],
  files: FileMap,
): FileMap {
